function setUpSpecialNavs() {
  ...
  $("body")
    ...
    .on("click", ".blocsapp-special-menu a", function (n) {
      $(n.target).closest(".dropdown-toggle").length ||
...
function getFillHeight() {
  var n = $(window).height();
  return n < fillBodyHeight && (n = fillBodyHeight + 100), n;
}
function scrollToTarget(n) {
  1 == n
    ...
    : ((n = $(n).offset().top),
      $(".sticky-nav").length &&
        (n -= $(".sticky-nav .navbar-header").height()));
...
function inViewCheck() {
  $($(".hideMe").get().reverse()).each(function () {
    ...
    (n.height() > $(window).height() && (t = n.offset().top), t < r) &&
      ...
      setTimeout(function () {
        n.attr("class", i).css("visibility", "visible");
      }, 0.01));
  });
}
...
function setUpVisibilityToggle() {
  $(document).on("click", "[data-toggle-visibility]", function (n) {
    function r(n) {
      n.is("img") ? n.toggle() : n.slideToggle();
    }
    var t, i;
    n.preventDefault();
...
function setUpLightBox() {
  ...
  $(document)
    .on("click", "[data-lightbox]", function (n) {
      ...
      "no-gallery-set" == t
        ...
        : (0 == $('a[data-gallery-id="' + t + '"]').index(targetLightbox) &&
            $(".prev-lightbox").hide(),
          $('a[data-gallery-id="' + t + '"]').index(targetLightbox) ==
            $('a[data-gallery-id="' + t + '"]').length - 1 &&
            $(".next-lightbox").hide());
  ...
  $(document).on("click", ".next-lightbox, .prev-lightbox", function (n) {
    ...
    targetLightbox.attr("data-gallery-id") &&
      ...
      (r = $('a[data-gallery-id="' + i + '"]').index(targetLightbox)),
    ...
    "no-gallery-set" == i
      ? ($("a[data-lightbox]").index(t) != $("a[data-lightbox]").length - 1 &&
          $(".next-lightbox").show(),
        $("a[data-lightbox]").index(t) > 0 && $(".prev-lightbox").show())
      : ($('a[data-gallery-id="' + i + '"]').index(t) !=
          $('a[data-gallery-id="' + i + '"]').length - 1 &&
          $(".next-lightbox").show(),
        $('a[data-gallery-id="' + i + '"]').index(t) > 0 &&
          $(".prev-lightbox").show());
  });
}
